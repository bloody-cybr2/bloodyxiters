...
function Do-CreateRestorePoint {
    try {
        ...
        Checkpoint-Computer -Description 'FLUXO-BYTE-OPTIMIZER' -RestorePointType 'MODIFY_SETTINGS'
...
function Do-ExportReport {
    try {
        ...
        $path = Join-Path $desktop ("fluxoOptimizer-Report-{0}.txt" -f (Get-Date -f 'yyyyMMdd-HHmmss'))
        $content = @()
        $content += 'FLUXO PC OPTIMIZER - REPORT'
...
$form.Text = 'FLUXO PC OPTIMIZER - DANGEROUS MODE'
...
$banner.Text = 'FLUXO PC OPTIMIZER'
